f = open("AoC_Inputs\Advent_14_22.txt").read().strip()
s = f.split("\n")
...
def simulate_sand(p1):
    ...
    while True:
        ...
        while True:
            if sand_y + 1 == max_y:
                if p1: return len(obstacles) - len(walls)
                else: 
                    obstacles.add((sand_x, sand_y)) 
                    break
            if (sand_x, sand_y + 1) not in obstacles: sand_y += 1
...
print(simulate_sand(True))
print(simulate_sand(False))
